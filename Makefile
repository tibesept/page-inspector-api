...
# --------------------------------------------------
logs: ## Показать логи всех сервисов в реальном времени либо конкретного сервиса (например, make logs srv=api)
	docker compose $(COMPOSE_FILES) logs -f --tail=100 $(srv)

ps: ## Показать статус контейнеров
	docker compose $(COMPOSE_FILES) ps

shell: ## Зайти в shell сервиса (например, make shell srv=api-1)
